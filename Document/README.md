...
**[dsovs.com](https://dsovs.com)** で実際に試すか、[`assessment/`](https://github.com/OWASP/www-project-devsecops-verification-standard/tree/main/assessment/assessment/) フォルダから実行します。すべての DSOVS コントロールに対して成熟度を評価し、レポートを生成します。これはブラウザ内で完全に実行するため、回答はデバイス上にローカルに保存され、外部送信することはありません。結果を JSON でエクスポートするか、PDF に出力できます。

独自のツールを構築していますか？すべてのコントロールは機械可読データとしても公開されています。[JSON API](https://raw.githubusercontent.com/OWASP/www-project-devsecops-verification-standard/refs/heads/main/dist/dsovs.json) は [`data/controls/`](https://github.com/OWASP/www-project-devsecops-verification-standard/tree/main/data/controls) にある信頼できるソースファイルから生成されます。
