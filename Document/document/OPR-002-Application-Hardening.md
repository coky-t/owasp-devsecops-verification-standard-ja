# アプリケーションの堅牢化 (Application Hardening)
...
アプリケーションの堅牢化は実行時に攻撃対象領域を減らすことによってアプリケーションのセキュリティを強化するプロセスです。This typically involves applying secure configuration defaults, removing features, sample content and debug endpoints that are not needed in production, enforcing strong transport security, and running the application with the least privilege it requires to function.
